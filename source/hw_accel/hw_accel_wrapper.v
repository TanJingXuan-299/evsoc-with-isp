...
module hw_accel_wrapper #(
   parameter DATA_WIDTH             = 32,  //For DMA and APB
   ...
   input  wire                      clk,
   input  wire                      rst,
   input  wire                      apb_slave_clk,
   input  wire                      apb_slave_rst,

   //APB slave-related signals
   input wire [7:0] i_sobel_thresh_var,
   input wire [1:0] i_hw_accel_mode,
   input wire       i_dma_wr_init_done,
   ...
   //APB slave write to HW accelerator
   always@(posedge apb_slave_clk or posedge apb_slave_rst)
   begin
      if (apb_slave_rst) begin
         sobel_thresh_val       <= SOBEL_THRESH;
         hw_accel_mode          <= 2'd0;
         hw_accel_dma_init_done <= 1'b0;
      end else begin
         //APB slave
         sobel_thresh_val       <= i_sobel_thresh_var;
         hw_accel_mode          <= i_hw_accel_mode;
         hw_accel_dma_init_done <= i_dma_wr_init_done;
      end
   end
