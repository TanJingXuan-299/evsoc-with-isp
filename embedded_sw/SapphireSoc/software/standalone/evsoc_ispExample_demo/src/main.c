...
void main()
{
    ...
    while (1)
    {
        ...
        if (select_demo_mode == 2 || select_demo_mode > 3)
        {
            ...
            // SET Sobel edge detection threshold via APB3 slave
            write_u32(5, EXAMPLE_APB3_SLV_HW + EXAMPLE_APB3_SLV_HW_REG0_OFFSET); // Default value 100; Range 0 to 255

            // SELECT HW accelerator mode - Make sure match with DMA transfer length setting
            if (select_demo_mode == 2 || select_demo_mode == 4)
            {
                write_u32(0x00000000, EXAMPLE_APB3_SLV_HW + EXAMPLE_APB3_SLV_HW_REG1_OFFSET); // 2'd0: Sobel only
            }
            else if (select_demo_mode == 5)
            {
                write_u32(0x00000001, EXAMPLE_APB3_SLV_HW + EXAMPLE_APB3_SLV_HW_REG1_OFFSET); // 2'd1: Sobel+Dilation
            }
            else
            {
